    ...
    window.AppCalendar = App.extend({
        handleFullcalendar: function () {
            /*
             ...
             start: '2016-10-01T11:30:00',
             end: '2016-10-01T12:30:00',
             ...
             */
            //var my_events = [];
            //var my_events = $('#valueAjax').attr('data-value');


            //alert(my_events);
            //var my_events = [{title: 'All Day Event', start: '2017-03-01T10:30:00', end: '2017-03-01T11:30:00',}];
            ...
            var my_options = {
                ...
                eventClick: function (event) {
                    if (event.diary_id > 0) {
                        $.ajax({
                            url: '/portal/condominium/diary/edit/' + event.diary_id,
                            method: 'GET',
                            dataType: 'html',
                            success: function (result) {
                                $('#editNewEvent .modal-body').html(result);
                                btnDelete();
                            }
                        });
                    }
                ...
                events: function (start, end, timezone, callback) {
                    $.ajax({
                        url: '/portal/condominium/diary/all',
                        dataType: 'json',
                        success: function (doc) {
                            var events = [];
                            $.each(doc, function (key, data) {
                                $.each(data, function (index, data) {
                                    var title = data.start_time + ' às ' + data.end_time + " - \n" + data.reserve_area.name;
                                    events.push({
                                        title: title,
                                        start: data.date,
                                        diary_id: data.id
                                        //backgroundColor: "#FF0000",
                                        //borderColor: "#000000" // will be parsed
                                    });
                                });
                            });
                            callback(events);
